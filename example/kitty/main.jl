...
function main()
    ...
    meshscatter!(
        ...
        color=:black, markersize=0.0075, quality=8,
    )

    # visualizer.pc_axis |> Makie.autolimits!
    xlims!(visualizer.pc_axis, (-5, 6))
    ylims!(visualizer.pc_axis, (25, 35))
    zlims!(visualizer.pc_axis, (0, 2))
...
function offliner()
    ...
    lines!(
        ...
        color=:green, quality=1, linewidth=5,
    )
    meshscatter!(
    ...
    for i in 1:length(slam_positions)
        ...
        xlims!(visualizer.pc_axis, (-5 + camera_pos[1], 5 + camera_pos[1]))
        ylims!(visualizer.pc_axis, (-5 + camera_pos[2], 5 + camera_pos[2]))
